...
var MyFuliye = React.createClass({
    getInitialState: function () {
        return {initTime: new Date().getTime(), currentTime: new Date().getTime(), showFirstXOnly: true};
    },
    componentDidMount: function() {
        ...
        $(this.refs.root).find('.showOrHide').click(function(){
            this.setState({showFirstXOnly:!this.state.showFirstXOnly});
        }.bind(this));
    },
    ...
    onXClick: function (item, e) {
        try {
            var newX = window.prompt("Change X to", parseFloat($(e.target).val()));
            if(!newX) {
                newX = parseFloat($(e.target).val());
            }
            item.theeter = newX;
        } catch (err) {
            console.log(err);
    ...
    onMarginClick: function (item, e) {
        try {
            var newMargin = window.prompt("Change Margin to", parseFloat($(e.target).val()));
            if(!newMargin) {
                newMargin = parseFloat($(e.target).val());
            }
    ...
    render: function() {
        ...
        var controlArea = <div className="buttons">
        ...
        var series = this.props.settings.series.map(function (item, i) {
            if(this.state.showFirstXOnly && i > 2) {
                return '';
            }
            ...
            return (
                <div key={item.id} className="aSeries">
                    <label className="seriesTitle" style={{position:'relative',top:-(totalHeight/2-15)+'px',fontWeight:'bold',color:item.stroke}}>
                        ...
                        <div>
                            x:<input type="text" value={item.theeter} style={{width:'18px'}} onChange={this.onXChange.bind(null,item)} onClick={this.onXClick.bind(null,item)} />
                            {' '}
                            margin:<input type="text" value={item.margin} style={{width:'40px'}} onChange={this.onMarginChange.bind(null,item)} onClick={this.onMarginClick.bind(null,item)}/>
                        </div>
                    </label>
