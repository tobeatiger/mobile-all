...
Cube3DCanvas.prototype.explodeBig = function(amount,duration) {
    if (amount==null) amount=2;
    if (duration==null) duration=2000;
...
Cube3DCanvas.prototype.explodeSmall = function(amount,duration) {
    if (amount==null) amount=2;
    if (duration==null) duration=2000;
    ...
    var start=new Date().getTime();
    var f=function() {
        var now=new Date().getTime()+0.308*2000;
        var elapsed=now-start;
        var x=elapsed/duration;
        if (x>0.308 && x<1) {
            self.repaint(f);
